...
import { ACADEMY_EXPERIENCE_OPTIONS, ACADEMY_TRACK_OPTIONS } from "@/lib/academy";
...
export default function InternshipApplyForm() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(
        `/academy/internship/apply/success?application_no=${encodeURIComponent(
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <h1 className="text-2xl font-extrabold text-slate-900 sm:text-3xl">Internship Application Form</h1>
      <p className="mt-2 text-sm text-slate-600">
        Fill this form and our academy team will contact you for the next batch process.
      </p>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <Input
          ...
          placeholder="Graduation / current year"
        />
        <Input
          label="Availability"
          value={form.availability}
          onChange={(value) => setForm((prev) => ({ ...prev, availability: value }))}
          placeholder="Example: 20 hrs/week"
        />
      </div>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <Select
          ...
          options={ACADEMY_EXPERIENCE_OPTIONS.map((value) => ({ value, label: value }))}
        />
      </div>

      <div className="mt-4">
        <Input
          label="Resume link (Drive/portfolio)"
          value={form.resumeLink}
          onChange={(value) => setForm((prev) => ({ ...prev, resumeLink: value }))}
          placeholder="https://..."
        />
      </div>
