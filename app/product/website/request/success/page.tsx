...
export default async function WebsiteRequestSuccessPage({ searchParams }: Props) {
  const params = (await searchParams) ?? {};
  const orderNo = pickParam(params.order_no);
  const requestNo = pickParam(params.request_no);
  ...
  return (
    <main className="min-h-screen bg-emerald-50 px-4 py-14 text-slate-900">
      <div className="mx-auto w-full max-w-2xl rounded-3xl border border-emerald-200 bg-white p-7 shadow-sm sm:p-9">
        ...
        <h1 className="mt-4 text-3xl font-extrabold text-slate-900">Payment successful</h1>
        <p className="mt-2 text-sm text-slate-600 sm:text-base">
          Your website request has been recorded. Our team will contact you with next steps shortly.
        </p>
        ...
        <div className="mt-6 space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm">
          <p>
            ...
            <span className="font-bold text-slate-900">{requestNo || "Generated"}</span>
          </p>
          <p>
            <span className="font-semibold text-slate-700">Order No:</span>{" "}
            <span className="font-bold text-slate-900">{orderNo || "Generated"}</span>
          </p>
        </div>
