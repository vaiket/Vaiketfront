...
export const ADMIN_SESSION_COOKIE = "vaiket_admin_session";
...
type SessionBody = Omit<AdminSessionPayload, "exp">;
...
export function readAdminSessionToken(token?: string | null): AdminSessionPayload | null {
  ...
  const payload = safeJsonParse<AdminSessionPayload>(
    Buffer.from(body, "base64url").toString("utf8")
  );

  if (!payload?.adminId || !payload?.email || !payload?.name || !payload?.exp) return null;
  if (payload.exp <= Math.floor(Date.now() / 1000)) return null;

  return payload;
}

export function getAdminCookieOptions(maxAgeSeconds = ADMIN_SESSION_TTL_SECONDS) {
  return {
    httpOnly: true,
